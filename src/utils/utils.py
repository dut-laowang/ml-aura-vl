...
def setup_wandb(args: argparse.Namespace) -> t.Any:
    if getattr(args, "wandb_enabled", 1) == 0:
        print("[INFO] wandb disabled by args.")
        return None  # 表示 wandb 不启用
    ...
    try:
        ...
        os.environ["WANDB_API_KEY"] = wandb_config.get("WANDB_API_KEY", "")
        os.environ["WANDB_BASE_URL"] = wandb_config.get("WANDB_BASE_URL", "")
    except FileNotFoundError:
        print("[WARNING] .wandb.yaml not found, proceeding with default wandb config.")
        wandb_config = {}
